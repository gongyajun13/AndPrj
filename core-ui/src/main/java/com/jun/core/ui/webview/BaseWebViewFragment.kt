 ...
 * 使用示例：
 * ```kotlin
 * class MyWebViewFragment : BaseWebViewFragment<FragmentWebViewBinding>(), WebViewCallback {
 ...
 *     
 *     override fun getInitialUrl(): String? = "https://example.com"
 * }
 * ```
 */
abstract class BaseWebViewFragment<VB : ViewBinding> : BaseFragment<VB>(), WebViewCallback {
    ...
    private fun initWebView() {
        ...
        // 创建或获取 WebView
        val webViewInstance = WebViewManager.createWebView(requireContext(), config)
    ...
    override fun onResume() {
        super.onResume()
        when (val wv = webView) {
            is SystemWebViewWrapper -> wv.onResume()
            is X5WebView -> wv.onResume()
        }
    }
    ...
    override fun onPause() {
        super.onPause()
        when (val wv = webView) {
            is SystemWebViewWrapper -> wv.onPause()
            is X5WebView -> wv.onPause()
        }
    }
     ...
     */
    fun goForward() {
        val canGoForward = when (val wv = webView) {
            is SystemWebViewWrapper -> wv.canGoForward()
            is X5WebView -> wv.canGoForward()
            else -> false
        }
        if (canGoForward) {
            when (val wv = webView) {
