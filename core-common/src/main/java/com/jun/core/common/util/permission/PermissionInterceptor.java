...
public final class PermissionInterceptor implements OnPermissionInterceptor {
    ...
    public void onRequestPermissionEnd(@NonNull Activity activity, boolean skipRequest,
        ...
        if (!doNotAskAgain) {
            ...
            // 使用 Activity 的根 View 作为 anchor，避免 getCurrentFocus() 返回 null
            View anchorView = null;
            try {
                anchorView = activity.getCurrentFocus();
                if (anchorView == null) {
                    anchorView = activity.findViewById(android.R.id.content);
                }
                if (anchorView == null && activity.getWindow() != null) {
                    anchorView = activity.getWindow().getDecorView();
                }
            } catch (Exception e) {
                // 忽略异常，使用备用方案
            }
            
            // 如果所有方法都返回 null，使用 Toast 直接显示（不依赖 View）
            if (anchorView != null) {
                UiNotifierManager.INSTANCE.info(anchorView, permissionHint);
            } else {
                // 备用方案：使用 Toast 直接显示
                android.widget.Toast.makeText(activity, permissionHint, android.widget.Toast.LENGTH_SHORT).show();
            }
            return;
        }
    ...
    private String generatePermissionHint(@NonNull Activity activity, @NonNull List<IPermission> deniedList, boolean doNotAskAgain) {
        ...
        return activity.getString(doNotAskAgain ? R.string.common_permission_fail_assign_hint_1 :
                                                ...
                                                PermissionConverter.INSTANCE.getNickNamesByPermissions(activity, deniedList));
    }
