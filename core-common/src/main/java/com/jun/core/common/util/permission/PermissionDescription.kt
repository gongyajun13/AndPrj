...
import com.jun.core.common.util.permission.PermissionConverter.getDescriptionsByPermissions
...
class PermissionDescription : OnPermissionDescription {
    ...
    override fun askWhetherRequestPermission(
        ...
        showDialog(
            activity,
            activity.getString(R.string.common_permission_description_title),
            generatePermissionDescription(activity, requestList),
            activity.getString(R.string.common_permission_confirm)
        ) { dialog, _ ->
            dialog.dismiss()
            continueRequestRunnable.run()
        }
    ...
    private fun showDialog(
        ...
        if (activity is AppCompatActivity) {
            mPermissionDialog = AlertDialog.Builder(activity)
            ...
            mPermissionDialog = android.app.AlertDialog.Builder(activity)
                .setTitle(dialogTitle)
                .setMessage(dialogMessage) // 对话框一定要设置成不可取消的
                .setCancelable(false)
                .setPositiveButton(confirmButtonText, confirmListener)
    ...
    private fun showPopupWindow(activity: Activity, content: String) {
        ...
        val messageView = mPermissionPopupWindow!!.getContentView()
